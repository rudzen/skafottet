...
public class ColorPickerView extends View {
    ...
    private void drawAlphaPanel(final Canvas canvas) {
        ...
        final float[] hsv = {mHue, mSat, mVal};
    ...
    private float[] pointToSatVal(float x, float y) {
        ...
        if (x < rect.left) {
            x = 0f;
        } else x = x > rect.right ? width : x - rect.left;

        if (y < rect.top) {
            y = 0f;
        } else y = y > rect.bottom ? height : y - rect.top;
    ...
    private int pointToAlpha(int x) {
        ...
        if (x < rect.left) {
            x = 0;
        } else x = x > rect.right ? width : x - (int) rect.left;
    ...
    private int chooseWidth(final int mode, final int size) {
        return mode == MeasureSpec.AT_MOST || mode == MeasureSpec.EXACTLY ? size : getPrefferedWidth();
    }

    private int chooseHeight(final int mode, final int size) {
        return mode == MeasureSpec.AT_MOST || mode == MeasureSpec.EXACTLY ? size : getPrefferedHeight();
    }
     ...
     */
    private void setAlphaSliderText(final String text) {
